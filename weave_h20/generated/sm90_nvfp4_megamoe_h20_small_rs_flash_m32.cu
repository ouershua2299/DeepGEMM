...
kernel_small_rs_dispatch_fragment(int num_tokens, LoomTensorMap const* l1_activation, LoomTensorMap const* l1_activation_scale, LoomTensorMap const* l1_packed_weights, LoomTensorMap const* l1_output, LoomTensorMap const* l2_activation, LoomTensorMap const* l2_activation_scale, LoomTensorMap const* l2_packed_weights, float* __restrict__ l1_global_scales, float* __restrict__ l2_global_scales, __nv_bfloat16* __restrict__ output_bf16, int32_t pg_world, int32_t pg_rank, unsigned* const* __restrict__ pg_flags, uint8_t* __restrict__ sym_buffer, uint8_t* const* __restrict__ sym_buffer_peers)
{
    ...
    float* l1_sfa_smem = reinterpret_cast<float*>(smem_raw + 17408);
    const int l1_sfa_smem_addr = smem + 17408;
    uint8_t* l1_packed_smem = reinterpret_cast<uint8_t*>(smem_raw + 18176);
    const int l1_packed_smem_addr = smem + 18176;
    unsigned int* l1_lut_smem = reinterpret_cast<unsigned int*>(smem_raw + 141056);
    const int l1_lut_smem_addr = smem + 141056;
    uint8_t* l1_decoded_smem = reinterpret_cast<uint8_t*>(smem_raw + 142336);
    const int l1_decoded_smem_addr = smem + 142336;
    uint8_t* l1_output_smem = reinterpret_cast<uint8_t*>(smem_raw + 142336);
    const int l1_output_smem_addr = smem + 142336;
    float* l1_amax_smem = reinterpret_cast<float*>(smem_raw + 143360);
    const int l1_amax_smem_addr = smem + 143360;
    __nv_bfloat16* l2_output_smem = reinterpret_cast<__nv_bfloat16*>(smem_raw + 142336);
    const int l2_output_smem_addr = smem + 142336;
