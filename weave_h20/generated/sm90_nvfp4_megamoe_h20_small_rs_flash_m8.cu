...
kernel_small_rs_dispatch_fragment(int num_tokens, LoomTensorMap const* l1_activation, LoomTensorMap const* l1_activation_scale, LoomTensorMap const* l1_packed_weights, LoomTensorMap const* l1_output, LoomTensorMap const* l2_activation, LoomTensorMap const* l2_activation_scale, LoomTensorMap const* l2_packed_weights, float* __restrict__ l1_global_scales, float* __restrict__ l2_global_scales, __nv_bfloat16* __restrict__ output_bf16, int32_t pg_world, int32_t pg_rank, unsigned* const* __restrict__ pg_flags, uint8_t* __restrict__ sym_buffer, uint8_t* const* __restrict__ sym_buffer_peers)
{
    ...
    float* l1_sfa_smem = reinterpret_cast<float*>(smem_raw + 13312);
    const int l1_sfa_smem_addr = smem + 13312;
    uint8_t* l1_packed_smem = reinterpret_cast<uint8_t*>(smem_raw + 14080);
    const int l1_packed_smem_addr = smem + 14080;
    unsigned int* l1_lut_smem = reinterpret_cast<unsigned int*>(smem_raw + 136960);
    const int l1_lut_smem_addr = smem + 136960;
    uint8_t* l1_decoded_smem = reinterpret_cast<uint8_t*>(smem_raw + 138240);
    const int l1_decoded_smem_addr = smem + 138240;
    uint8_t* l1_output_smem = reinterpret_cast<uint8_t*>(smem_raw + 138240);
    const int l1_output_smem_addr = smem + 138240;
    float* l1_amax_smem = reinterpret_cast<float*>(smem_raw + 139264);
    const int l1_amax_smem_addr = smem + 139264;
    __nv_bfloat16* l2_output_smem = reinterpret_cast<__nv_bfloat16*>(smem_raw + 138240);
    const int l2_output_smem_addr = smem + 138240;
                ...
                while (true) {
                    ...
                    if (elect_sync()) {
                        ...
                        *(reinterpret_cast<int*>(reinterpret_cast<int*>(reinterpret_cast<uint8_t*>(sym_buffer) + 69827968)) + (metadata_idx + 2)) = (int)src_topk_idx;
                        ...
                        asm volatile("red.release.gpu.global.add.u32 [%0], %1;" :: "l"(&reinterpret_cast<unsigned int*>(reinterpret_cast<uint8_t*>(sym_buffer) + 4480)[expert_pool_block_offset + token_idx_in_expert / 8]), "r"((unsigned int)(1)) : "memory");
                    }
                    __syncwarp();
                        ...
                        for (int token_chunk_2 = 0; token_chunk_2 < 1; token_chunk_2++) {
                            ...
                            for (int half = 0; half < 2; half++) {
                                if (token_0_2 < math_task_valid_m) {
                                    ...
                                    {
                                        _vec_load_10[0] = *reinterpret_cast<const float*>(reinterpret_cast<float*>(reinterpret_cast<uint8_t*>(sym_buffer) + 2640225664) + m_idx + token_0_2);
                                    }
                                ...
                                if (token_1_2 < math_task_valid_m) {
                                    ...
                                    {
                                        _vec_load_11[0] = *reinterpret_cast<const float*>(reinterpret_cast<float*>(reinterpret_cast<uint8_t*>(sym_buffer) + 2640225664) + m_idx + token_1_2);
                                    }
            ...
            for (unsigned int combine_token_idx = bid * 8 + epilogue_warp_idx_2; combine_token_idx < num_tokens; combine_token_idx += 624) {
                ...
                if (lane < 6) {
                    long long _vec_load_16[1];
                    {
                        ...
                        asm volatile("ld.global.nc.b64 %0, [%1];"
                            : "=l"(_scalar_bits_10) : "l"((const void*)(reinterpret_cast<long long*>(reinterpret_cast<uint8_t*>(sym_buffer) + 110452096) + (combine_token_idx * 6 + lane))) : "memory");
                ...
                for (int combine_chunk = 0; combine_chunk < 1; combine_chunk++) {
                    ...
                    if (elect_sync()) {
                        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                        {
                            void* _cpbulk_dst_11 = reinterpret_cast<void*>(output_bf16 + ((unsigned long long)combine_token_idx * 4096 + (unsigned long long)(chunk_byte_offset / 2)));
                            asm volatile(
                                "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
                                :: "l"(_cpbulk_dst_11), "r"(combine_store_smem_addr + (unsigned int)(epilogue_warp_idx_2 * 8192)), "r"((uint32_t)(8192))
                                : "memory");
                        }
                        asm volatile("cp.async.bulk.commit_group;");
                    }
