...
kernel_split_l2_fragment(int num_tokens, LoomTensorMap const* l2_activation, LoomTensorMap const* l2_activation_scale, LoomTensorMap const* l2_packed_weights, float* __restrict__ l2_global_scales, __nv_bfloat16* __restrict__ output_bf16, int32_t pg_world, int32_t pg_rank, unsigned* const* __restrict__ pg_flags, uint8_t* __restrict__ sym_buffer, uint8_t* const* __restrict__ sym_buffer_peers)
{
    ...
    unsigned int* lut_smem = reinterpret_cast<unsigned int*>(smem_raw + 2048);
    const int lut_smem_addr = smem + 2048;
    uint8_t* cd_smem = reinterpret_cast<uint8_t*>(smem_raw + 3072);
    const int cd_smem_addr = smem + 3072;
    uint8_t* a_smem = reinterpret_cast<uint8_t*>(smem_raw + 20480);
    const int a_smem_addr = smem + 20480;
    uint8_t* b_packed_smem = reinterpret_cast<uint8_t*>(smem_raw + 118784);
    const int b_packed_smem_addr = smem + 118784;
    uint8_t* b_decoded_smem = reinterpret_cast<uint8_t*>(smem_raw + 118784);
    const int b_decoded_smem_addr = smem + 118784;
    float* sfa_smem = reinterpret_cast<float*>(smem_raw + 217088);
    const int sfa_smem_addr = smem + 217088;
        ...
        { // loader_main
            ...
            for (int expert_group = 0; expert_group < 1; expert_group++) {
                ...
                if (local_expert < 32) {
                    ...
                    asm volatile("ld.volatile.global.b64 %0, [%1];" : "=l"(_sys_volatile_0) : "l"(reinterpret_cast<const unsigned long long*>(&reinterpret_cast<unsigned long long*>(reinterpret_cast<uint8_t*>(sym_buffer) + 4224)[local_expert])) : "memory");
                    ready_status = _sys_volatile_0;
                    while ((unsigned int)(ready_status >> 32) != 624) {
                        unsigned long long _sys_volatile_1;
                        asm volatile("ld.volatile.global.b64 %0, [%1];" : "=l"(_sys_volatile_1) : "l"(reinterpret_cast<const unsigned long long*>(&reinterpret_cast<unsigned long long*>(reinterpret_cast<uint8_t*>(sym_buffer) + 4224)[local_expert])) : "memory");
            ...
            while (state[1] < 32) {
                ...
                while (scanning != 0) {
                    if (state[1] < 32) {
                        ...
                        if (state[0] < num_m_blocks * 32) {
                            state[5] = state[0] / 32;
                            state[6] = state[0] - state[5] * 32;
                            state[0] = state[0] + 78;
                            state[4] = 1;
                            scanning = 0;
                        } else {
                            state[0] = state[0] - num_m_blocks * 32;
                ...
                if (state[4] != 0) {
                    ...
                    int valid_m = _min_0;
                    int weight_row = (int)(state[1] * 4096 + state[6] * 128);
                    #pragma unroll 1
                    for (int k_block_idx = 0; k_block_idx < 16; k_block_idx++) {
        ...
        { // decode_main
            ...
            for (int expert_group_3 = 0; expert_group_3 < 1; expert_group_3++) {
                ...
                if (local_expert_1 < 32) {
                    ...
                    asm volatile("ld.volatile.global.b64 %0, [%1];" : "=l"(_sys_volatile_2) : "l"(reinterpret_cast<const unsigned long long*>(&reinterpret_cast<unsigned long long*>(reinterpret_cast<uint8_t*>(sym_buffer) + 4224)[local_expert_1])) : "memory");
                    ready_status_1 = _sys_volatile_2;
                    while ((unsigned int)(ready_status_1 >> 32) != 624) {
                        unsigned long long _sys_volatile_3;
                        asm volatile("ld.volatile.global.b64 %0, [%1];" : "=l"(_sys_volatile_3) : "l"(reinterpret_cast<const unsigned long long*>(&reinterpret_cast<unsigned long long*>(reinterpret_cast<uint8_t*>(sym_buffer) + 4224)[local_expert_1])) : "memory");
            ...
            #pragma unroll
            for (int expert_group_4 = 0; expert_group_4 < 1; expert_group_4++) {
            ...
            while (state_d[1] < 32) {
                ...
                while (scanning_1 != 0) {
                    if (state_d[1] < 32) {
                        ...
                        if (state_d[0] < num_m_blocks_1 * 32) {
                            state_d[5] = state_d[0] / 32;
                            state_d[6] = state_d[0] - state_d[5] * 32;
                            state_d[0] = state_d[0] + 78;
                            state_d[4] = 1;
                            scanning_1 = 0;
                        } else {
                            state_d[0] = state_d[0] - num_m_blocks_1 * 32;
                            state_d[3] = state_d[3] + num_m_blocks_1;
                            state_d[1] = state_d[1] + 1;
                            unsigned int next_tokens_1[1];
                            unsigned int lane_value_0_1 = 0;
                            #pragma unroll
                            for (int expert_group_5 = 0; expert_group_5 < 1; expert_group_5++) {
                ...
                if (state_d[4] != 0) {
                    #pragma unroll 1
                    for (int _k_block_idx = 0; _k_block_idx < 16; _k_block_idx++) {
    ...
    if (warp >= 4 && warp <= 11) {
        { // math_main
            ...
            unsigned int stored_m[1];
            #pragma unroll
            for (int expert_group_6 = 0; expert_group_6 < 1; expert_group_6++) {
                int local_expert_2 = (unsigned int)(expert_group_6 * 32) + lane;
                unsigned long long ready_status_2 = 0;
                if (local_expert_2 < 32) {
                    unsigned long long _sys_volatile_4;
                    asm volatile("ld.volatile.global.b64 %0, [%1];" : "=l"(_sys_volatile_4) : "l"(reinterpret_cast<const unsigned long long*>(&reinterpret_cast<unsigned long long*>(reinterpret_cast<uint8_t*>(sym_buffer) + 4224)[local_expert_2])) : "memory");
                    ready_status_2 = _sys_volatile_4;
                    while ((unsigned int)(ready_status_2 >> 32) != 624) {
                        unsigned long long _sys_volatile_5;
                        asm volatile("ld.volatile.global.b64 %0, [%1];" : "=l"(_sys_volatile_5) : "l"(reinterpret_cast<const unsigned long long*>(&reinterpret_cast<unsigned long long*>(reinterpret_cast<uint8_t*>(sym_buffer) + 4224)[local_expert_2])) : "memory");
            ...
            #pragma unroll
            for (int expert_group_7 = 0; expert_group_7 < 1; expert_group_7++) {
            ...
            while (state_m[1] < 32) {
                ...
                while (scanning_2 != 0) {
                    if (state_m[1] < 32) {
                        ...
                        if (state_m[0] < num_m_blocks_2 * 32) {
                            state_m[5] = state_m[0] / 32;
                            state_m[6] = state_m[0] - state_m[5] * 32;
                            state_m[0] = state_m[0] + 78;
                            state_m[4] = 1;
                            scanning_2 = 0;
                        } else {
                            state_m[0] = state_m[0] - num_m_blocks_2 * 32;
                            state_m[3] = state_m[3] + num_m_blocks_2;
                            state_m[1] = state_m[1] + 1;
                            unsigned int next_tokens_2[1];
                            unsigned int lane_value_0_2 = 0;
                            #pragma unroll
                            for (int expert_group_8 = 0; expert_group_8 < 1; expert_group_8++) {
                ...
                if (state_m[4] != 0) {
                    ...
                    if (row_block_offset >= valid_m_m) {
                        #pragma unroll 1
                        for (int _k_skip = 0; _k_skip < 16; _k_skip++) {
                        ...
                        final_accum[63] = 0.0f;
                        #pragma unroll 1
                        for (int _k_block_idx_1 = 0; _k_block_idx_1 < 16; _k_block_idx_1++) {
                            mbarrier_wait(stage_dequant_addr + (math_stage) * 8, _phase_stage_dequant);
                            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                        ...
                        for (int j = 0; j < 4; j++) {
                            ...
                            if (lane_in_row == 0) {
                                ...
                                int _vec_load_1[1];
                                {
                                    _vec_load_1[0] = *reinterpret_cast<const int*>(reinterpret_cast<int*>(reinterpret_cast<uint8_t*>(sym_buffer) + 69827968) + metadata_base);
                                }
                                int _vec_load_2[1];
                                {
                                    _vec_load_2[0] = *reinterpret_cast<const int*>(reinterpret_cast<int*>(reinterpret_cast<uint8_t*>(sym_buffer) + 69827968) + metadata_base + 1);
                                }
                                int _vec_load_3[1];
                                {
                                    _vec_load_3[0] = *reinterpret_cast<const int*>(reinterpret_cast<int*>(reinterpret_cast<uint8_t*>(sym_buffer) + 69827968) + metadata_base + 2);
                                }
                            ...
                            if (row_offset < valid_m_m) {
                                ...
                                reinterpret_cast<int4*>(reinterpret_cast<unsigned int*>(reinterpret_cast<uint8_t*>(sym_buffer_peers[dst_rank]) + 4327982464) + dst_word_offset)[0] = reinterpret_cast<int4*>(packed)[0];
                            }
                        }
                        __syncwarp();
                        ...
                        for (int j_1 = 0; j_1 < 4; j_1++) {
                            ...
                            if (lane_in_row_1 == 0) {
                                ...
                                int _vec_load_4[1];
                                {
                                    _vec_load_4[0] = *reinterpret_cast<const int*>(reinterpret_cast<int*>(reinterpret_cast<uint8_t*>(sym_buffer) + 69827968) + metadata_base_1);
                                }
                                int _vec_load_5[1];
                                {
                                    _vec_load_5[0] = *reinterpret_cast<const int*>(reinterpret_cast<int*>(reinterpret_cast<uint8_t*>(sym_buffer) + 69827968) + metadata_base_1 + 1);
                                }
                                int _vec_load_6[1];
                                {
                                    _vec_load_6[0] = *reinterpret_cast<const int*>(reinterpret_cast<int*>(reinterpret_cast<uint8_t*>(sym_buffer) + 69827968) + metadata_base_1 + 2);
                                }
                            ...
                            if (row_offset_1 < valid_m_m) {
                                unsigned long long dst_word_offset_1 = (((unsigned long long)dst_topk_1 * 8448 + (unsigned long long)dst_token_1) * 4096 + (unsigned long long)n_idx + (unsigned long long)lane_in_row_1 * 8) / 2;
                                reinterpret_cast<int4*>(reinterpret_cast<unsigned int*>(reinterpret_cast<uint8_t*>(sym_buffer_peers[dst_rank_1]) + 4327982464) + dst_word_offset_1)[0] = reinterpret_cast<int4*>(packed_1)[0];
                            }
                        }
                        asm volatile("bar.sync 2, 256;" ::: "memory");
                    }
                ...
                if (lane < 6) {
                    long long _vec_load_7[1];
                    {
                        ...
                        asm volatile("ld.global.nc.b64 %0, [%1];"
                            : "=l"(_scalar_bits_6) : "l"((const void*)(reinterpret_cast<long long*>(reinterpret_cast<uint8_t*>(sym_buffer) + 110452096) + (combine_token_idx * 6 + lane))) : "memory");
                ...
                for (int combine_chunk = 0; combine_chunk < 1; combine_chunk++) {
                    ...
                    if (combine_mask != 0) {
                        ...
                        if (elect_sync()) {
                            ...
                            cp_async_bulk_gmem2smem(combine_load_smem_addr + load_barrier_stage * 8192, reinterpret_cast<uint8_t*>(sym_buffer) + (4327982464 + ((unsigned long long)slot_idx * 8448 + (unsigned long long)combine_token_idx) * 4096 * 2 + chunk_byte_offset), 8192, combine_full_addr + (load_barrier_stage) * 8);
                        }
                        __syncwarp();
                        do_reduce = 1;
                    }
                    ...
                    for (int combine_vec_1 = 0; combine_vec_1 < 16; combine_vec_1++) {
                        ...
                        for (int cast_word = 0; cast_word < 4; cast_word++) {
                            asm volatile("st.shared.b32 [%0], %1;" :: "r"(combine_store_smem_addr + (unsigned int)(epilogue_warp_idx * 8192) + ((unsigned int)(combine_vec_1 * 32) + lane) * 16 + (unsigned int)(cast_word * 4)), "r"((casted[cast_word])));
                        }
                    }
                    __syncwarp();
                    if (elect_sync()) {
                        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
                        {
                            void* _cpbulk_dst_7 = reinterpret_cast<void*>(output_bf16 + ((unsigned long long)combine_token_idx * 4096 + (unsigned long long)(chunk_byte_offset / 2)));
                            asm volatile(
                                "cp.async.bulk.global.shared::cta.bulk_group [%0], [%1], %2;"
                                :: "l"(_cpbulk_dst_7), "r"(combine_store_smem_addr + (unsigned int)(epilogue_warp_idx * 8192)), "r"((uint32_t)(8192))
                                : "memory");
                        }
            ...
            if (bid == 0) {
                #pragma unroll 1
                for (int expert_1 = epilogue_thread_idx; expert_1 < 256; expert_1 += 256) {
                ...
                #pragma unroll 1
                for (int cleanup_expert = bid - 1; cleanup_expert < 32; cleanup_expert += 77) {
                    unsigned long long _vec_load_8[1];
                    {
                        _vec_load_8[0] = *reinterpret_cast<const unsigned long long*>(reinterpret_cast<unsigned long long*>(reinterpret_cast<uint8_t*>(sym_buffer) + 4224) + cleanup_expert);
                    }
                    ...
                    #pragma unroll
                    for (int expert_group_9 = 0; expert_group_9 < 1; expert_group_9++) {
                    ...
                    if (epilogue_thread_idx == 0) {
                        *(reinterpret_cast<unsigned long long*>(reinterpret_cast<unsigned long long*>(reinterpret_cast<uint8_t*>(sym_buffer) + 4224)) + (cleanup_expert)) = 0;
                    }
                    if (epilogue_thread_idx < 8) {
                        *(reinterpret_cast<unsigned long long*>(reinterpret_cast<unsigned long long*>(reinterpret_cast<uint8_t*>(sym_buffer) + 2176)) + (epilogue_thread_idx * 32 + cleanup_expert)) = 0;
                    }
                    #pragma unroll 1
                    for (unsigned int cleanup_block = (unsigned int)epilogue_thread_idx; cleanup_block < cleanup_num_m_blocks; cleanup_block += 256) {
                        *(reinterpret_cast<unsigned int*>(reinterpret_cast<unsigned int*>(reinterpret_cast<uint8_t*>(sym_buffer) + 4480)) + (cleanup_pool_block_offset[0] + cleanup_block)) = 0;
                    }
                    asm volatile("bar.sync 2, 256;" ::: "memory");
                }
            }
