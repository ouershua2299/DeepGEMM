...
select_sm90_nvfp4_h200_fused(
    ...
    // The upstream MiMo table uses target wave sizes.  Flash has 32 local
    // experts rather than 48, so round the target upward to the nearest legal
    // divisor while keeping the same tile/stage policy for the A/B experiment.
    ...
    // The two added shapes have different dispatch/combine footprints.  Use
    // the legal H200 maximum for the isolated experiment so both scheduler
    // arms receive exactly the same one-CTA-per-SM capacity.
    const bool is_upstream_mimo =
        input.num_experts == 384 && input.num_topk == 8 &&
        input.hidden == 6144 && input.intermediate_hidden == 2048;
    if (!is_upstream_mimo)
        tuning.smem_size = SM90ArchSpec::smem_capacity;

    // Pro's wider dispatch/combine footprint makes the MiMo BM8/stage4 ring
    // exceed Hopper's 232448-byte CTA limit.  Match the existing local Pro
    // policy and fall back to stage3 for both A/B scheduler arms.
    const bool is_pro =
